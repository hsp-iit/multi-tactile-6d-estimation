...
bool LoadConfigurationFile(std::string config_file_path)
{
    ...
    while(!config_file.eof())
    {
        ...
        {
            config_file >> number_of_meshes;
            std::cout<<"Number of meshes : " + std::to_string(number_of_meshes)<<std::endl;
        }
        if (param == "NUMBER_OF_SENSORS")
        {
            config_file >> number_of_sensors;
            std::cout<<"Number of sensors : " + std::to_string(number_of_sensors)<<std::endl;
        }
        if (param == "MESH_PATH")
        {
            config_file >> mesh_path;
        ...
        if (param == "LANDMARKS_PATH")
        {
            ...
            std::cout<<"Poses sensors path : " + landmarks_path<<std::endl;;
        }
...
int main (int argc, char** argv)
{
    ...
    if (argc < 2)
    {
        std::cerr<< "Error! Insert config_file_path"<<std::endl;
        return 1;
    }

    std::string config_file_path = argv[1];

    if (!LoadConfigurationFile(config_file_path))
    {
    ...
    for (int iter = 0; iter < number_of_meshes; iter++)
    {
        ...
        obj_iter.loadObj((mesh_path + std::to_string(iter) + ".obj").c_str(), false);
        ...
        {
                PxVec3 v(obj_iter.mVertices[i*3], obj_iter.mVertices[i*3+1], obj_iter.mVertices[i*3+2]);
                vertices_iter[i] = v;
    ...
    for (int i = 0; i < vectors_sensors.size(); i++)
    {
        ...
        PxQuat quaternion(vectors_sensors[i][6], PxVec3(vectors_sensors[i][3], vectors_sensors[i][4], vectors_sensors[i][5]));
        PxMat33 matrix (quaternion);

        // Add the necessary translation due to the different position of the object reference system
...
#pragma omp parallel for
		for(int i =0; i < vectors.size(); i++)
		{
            ...
            for (int ns = 0; ns < number_of_sensors; ns++)
            {
			...
			// Assign the rotation matrix
			float norm = std::sqrt(std::pow(vectors[i][3], 2) + std::pow(vectors[i][4], 2) + std::pow(vectors[i][5], 2));
			PxQuat quaternion(norm, PxVec3(vectors[i][3]/norm, vectors[i][4]/norm, vectors[i][5]/norm));
			PxMat33 matrix (quaternion);
            ...
            for(int z = 0; z < number_of_meshes; z++)
            {
                ...
                for (int sens = 0; sens < number_of_sensors; sens++)
                {
                    ...
                    isPen[z] = PxGeometryQuery::computePenetration(direction_vectors[sens][z], depth_vectors[sens][z], convex_geom_vector[z], temp_transform, box_digit, sensors_transformation[sens]);
                    if(isPen[z])
                    {
                       // Check whether this sensor has a greater interpenetration than the others
